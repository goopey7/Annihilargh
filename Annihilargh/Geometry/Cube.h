...
namespace Geometry
{
	...
	public:
		...
		template <class V>
		static IndexedTriangleList<V> CreateTextured()
		{
			namespace dx = DirectX;

			constexpr float side = 1.0f / 2.0f;

			// must consider the vertices of a 2D texture for a cube
			...
			vertices[8].tex = {2.0f / 3.0f, 1.f};
			vertices[9].pos = {side, -side, -side};
			vertices[9].tex = {1.0f / 3.0f, 1.f};
			vertices[10].pos = {-side, -side, -side};
			vertices[10].tex = {1.f, 1.0f / 4.0f};
			vertices[11].pos = {-side, -side, side};
			vertices[11].tex = {1.f, 2.0f / 4.0f};
		...
		// generates a cube with independent faces, so each face gets a normal
		template <class V>
		static IndexedTriangleList<V> CreateIndependent()
		{
			constexpr float side = 1.0f / 2.0f;
